...
init -1098 python:
    # FS: add to keymap instead of overwriting it, and only in developer mode
    if config.developer:
        config.underlay.append(renpy.Keymap(action_editor=_viewers.action_editor))


init 1100 python:
    # FS: only in developer mode
    if config.developer:
        config.locked = False
